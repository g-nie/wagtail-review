...
class TestAdminViews(TestCase):
    ...
    def test_submit_for_review_action(self):
        ...
        response = self.client.get("/admin/pages/%d/edit/" % self.homepage.pk)  # noqa: UP031
    ...
    def test_submit_for_review_action_on_create(self):
        ...
        response = self.client.get(
            "/admin/pages/add/tests/simplepage/%d/" % self.homepage.pk  # noqa: UP031
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_validate_reviewers_required(self):
        ...
        response = self.client.post(
            "/admin/wagtail_review/create_review/",
            {
        ...
        self.assertFormSetError(
            response.context["reviewer_formset"],
            None,
            None,
            "Please select one or more reviewers.",
        )
    ...
    def test_post_edit_form(self):
        response = self.client.post(
            "/admin/pages/2/edit/",
            {
                ...
                "create_review_reviewers-0-email": "someone@example.com",
                "create_review_reviewers-0-DELETE": "",
                "create_review_reviewers-1-user": User.objects.get(
                    username="spongebob"
                ).pk,
        ...
        reviewer_emails = set(  # noqa: C401
            reviewer.get_email_address() for reviewer in review.reviewers.all()
        )
        self.assertEqual(
            reviewer_emails,
            {"admin@example.com", "someone@example.com", "spongebob@example.com"},
        )

        self.assertEqual(len(mail.outbox), 2)
        email_recipients = set(email.to[0] for email in mail.outbox)  # noqa: C401
        self.assertEqual(
            email_recipients, {"someone@example.com", "spongebob@example.com"}
        )
    ...
    def test_reviews_index(self):
        ...
        review = Review.objects.create(
            page_revision=revision, submitter=self.admin_user
        )
        ...
        self.assertContains(
            response,
            '<a href="/admin/wagtail_review/reviews/%d/">Home</a>' % self.homepage.pk,  # noqa: UP031
            html=True,
        )
    ...
    def test_review_audit_trail(self):
        ...
        review.reviewers.create(user=User.objects.get(username="spongebob"))
        response = self.client.get(
            "/admin/wagtail_review/reviews/%d/" % self.homepage.pk  # noqa: UP031
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_view_review(self):
        ...
        review.reviewers.create(user=self.admin_user)
        review.reviewers.create(user=User.objects.get(username="spongebob"))
        response = self.client.get("/admin/wagtail_review/reviews/%d/view/" % review.pk)  # noqa: UP031
        self.assertEqual(response.status_code, 200)
