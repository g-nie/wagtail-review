...
class TestReviewerModel(TestCase):
    ...
    def setUp(self):
        ...
        self.review = Review.objects.create(
            page_revision=self.revision, submitter=User.objects.first()
        )
    ...
    def test_tokens_are_assigned(self):
        ...
        self.assertRegex(reviewer.response_token, r"^\w{16}$")
        self.assertRegex(reviewer.view_token, r"^\w{16}$")
    ...
    def test_get_email_address(self):
        ...
        reviewer2 = Reviewer(
            review=self.review, user=User.objects.get(username="spongebob")
        )
    ...
    def test_get_respond_url(self):
        ...
        self.assertEqual(
            ...
            "/review/respond/%d/%s/" % (reviewer.id, reviewer.response_token),  # noqa: UP031
        )
        self.assertEqual(
            reviewer.get_respond_url(absolute=True),
            "http://test.local/review/respond/%d/%s/"  # noqa: UP031
            % (reviewer.id, reviewer.response_token),
        )
    ...
    def test_get_view_url(self):
        ...
        self.assertEqual(
            ...
            "/review/view/%d/%s/" % (reviewer.id, reviewer.view_token),  # noqa: UP031
        )
        self.assertEqual(
            reviewer.get_view_url(absolute=True),
            "http://test.local/review/view/%d/%s/" % (reviewer.id, reviewer.view_token),  # noqa: UP031
        )
