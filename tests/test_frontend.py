...
class TestFrontendViews(TestCase):
    ...
    def setUp(self):
        ...
        self.review = Review.objects.create(
            page_revision=submitted_revision, submitter=self.admin_user
        )
    ...
    def test_view_token_must_match(self):
        response = self.client.get("/review/view/%d/xxxxx/" % self.reviewer.id)  # noqa: UP031
        self.assertEqual(response.status_code, 403)

    def test_view(self):
        response = self.client.get(
            "/review/view/%d/%s/" % (self.reviewer.id, self.reviewer.view_token)  # noqa: UP031
        )
    ...
    def test_response_token_must_match(self):
        response = self.client.get("/review/respond/%d/xxxxx/" % self.reviewer.id)  # noqa: UP031
        self.assertEqual(response.status_code, 403)

    def test_respond_view(self):
        response = self.client.get(
            "/review/respond/%d/%s/" % (self.reviewer.id, self.reviewer.response_token)  # noqa: UP031
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "<h1>Simple page submitted</h1>")
        self.assertContains(response, "var app = new annotator.App();")
