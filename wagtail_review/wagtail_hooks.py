...
class SubmitForReviewMenuItem(ActionMenuItem):
    ...
    class Media:
        ...
        css = {"all": ["wagtail_review/css/create_review.css"]}
...
def handle_submit_for_review(request, page):
    if "action-submit-for-review" in request.POST:
        ...
        review = Review(
            page_revision=page.get_latest_revision(), submitter=request.user
        )
        form = ReviewForm(request.POST, instance=review, prefix="create_review")
        reviewer_formset = ReviewerFormSet(
            request.POST, instance=review, prefix="create_review_reviewers"
        )
